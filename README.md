...
Link OpenAMS Python modules to Klipper's extras directory
Add HDC1080 temperature sensor support
...
***THE FOLLOWING STEP MUST BE DONE LAST TO UPDATE TO CURRENT WORKING VERSION - WILL REMOVE WHEN UPSTREAM UPDATED Important: After installation, copy the UPDATED AFC integration modules to your Klipper AFC add-on extras folder:
...
MCU UUIDs: You must update the canbus_uuid or serial: /dev/serial/by-id/ values with your actual hardware UUIDs. Find them with:
...
Read the AFC Documentation: Most of the Armored Turtle setup documentation applies directly to OpenAMS. Review it at: https://www.armoredturtle.xyz/docs/boxturtle/initial_startup/01-overview.html

Box Turtle vs OpenAMS: Some documentation is specific to Box Turtle hardware and won't apply to OpenAMS. When in doubt:

Ask for clarification
Leave default settings in place
The OpenAMS-specific config files override Box Turtle defaults
Installation Prompts: The AFC installer doesn't list an OpenAMS-specific unit type. When prompted:
...
During Unload (SAFE_UNLOAD_FILAMENT):
...
Copy the Spoolman LED sync configuration file:
cp spoolman_led_sync.cfg ~/printer_data/config/AFC/
Edit ~/printer_data/config/AFC/spoolman_led_sync.cfg and enable the feature:
[spoolman_led_sync]
enable: True
default_color: 0000FF  # Blue (default) - used when no Spoolman data available
Optionally customize the default LED color for lanes without Spoolman data:
...
The module checks if the lane has a spool assigned in Spoolman
If a spool is found, it reads the filament color from Spoolman
The active tool LED is set to that color instead of the default blue
If no Spoolman data exists, it uses the configured default_color
Other lanes (not loaded) continue using their standard AFC LED colors
Mainsail AFC Panel
...
Ensure your AMS unit has at least one spool loaded

Run the calibration command via the Klipper console:

AFC_CALIBRATION
When prompted, select your OpenAMS unit from the list

The calibration process will automatically:

Measure PTFE tube lengths for each lane
Calibrate HUB_HES sensor values
Store the configuration values in your config file
Once calibration completes, restart Klipper to load the new settings:
...
Navigate to the AFC panel
Select the primary lane
Set the runout lane in the configuration dropdown
Multi-Lane Chains:
...
Verify Spoolman integration is active in AFC
Check that enable: True in spoolman_led_sync.cfg
Ensure lanes have spools assigned in Spoolman
Check Klipper logs for spoolman_led_sync errors
