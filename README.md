...
3. **Installation Prompts**: The AFC installer doesn't list an OpenAMS-specific unit type. When prompted:
   ...
   - After installation, **delete** the generated `AFC_Turtle_1.cfg` file:
     ...
     ```
     You may also need to delete the AFC.var.unit file that is autogenerated on startup as it will have the wrong info and may cause issues with the system prepping on load.
     ```bash
